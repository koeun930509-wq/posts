...
async function fetchPost() {
  ...
  try {
    ...
    writerEl.value = post.writer;
    // content에는 텍스트와 <img> 태그가 섞여 있을 수 있어, 위험한 태그/속성을 제거한 뒤 그대로 삽입
...
async function handleSubmit(event) {
  ...
  try {
    // id에 해당하는 게시글을 새 내용으로 수정 (update)
    const { error } = await supabaseClient
      .from('posts')
      .update({ title, writer, content, updated_at: new Date().toISOString() })
      .eq('id', postId);

    // Supabase가 에러를 반환한 경우 (권한 문제, 제약조건 위반 등)
    if (error) {
      console.error('게시글 수정 오류:', error);
      showError(`게시글 수정 중 오류가 발생했습니다: ${error.message}`);
      return;
    }

    // 수정 성공: 안내 메시지를 보여준 뒤 상세 페이지로 이동
    alert('게시글이 수정되었습니다.');
...
editFormEl.addEventListener('submit', handleSubmit);

// 취소 버튼 클릭 시 상세 페이지로 이동 (수정 취소)
cancelBtnEl.addEventListener('click', () => {
  window.location.href = `detail.html?id=${postId}`;
});

// 페이지 로드 시 기존 게시글 내용을 불러옴
fetchPost();
